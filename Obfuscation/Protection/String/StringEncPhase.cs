...
using Obfuscation.Protection.Renamer;
using Obfuscation.Services;

namespace Obfuscation.Protection.String
{
    public static class StringEncPhase
    {
        private static readonly string Key1 = RenamerPhase.GenerateString(RenamerPhase.RenameMode.Key);
        private static readonly string Key2 = RenamerPhase.GenerateString(RenamerPhase.RenameMode.Key);
        private static readonly string Key3 = RenamerPhase.GenerateString(RenamerPhase.RenameMode.Key);
        ...
        private static void InjectClass(ModuleDef module)
        {
            ...
            ObfuscateExe.Init = (MethodDef)dnlibDefs.Single(method => method.Name == "Decrypt");
            var cctor = module.GlobalType.FindStaticConstructor();
            ObfuscateExe.Init2 = (MethodDef)dnlibDefs.Single(method => method.Name == "Search");
        ...
        {
            InjectClass(module);
            var p = RenamerPhase.GenerateString(RenamerPhase.RenameMode.Normal);
                ...
                foreach (var meth in type.Methods)
                {
                    ...
                    for (var i = 0; i < instr.Count; i++)
                    {
                        if (instr[i].OpCode == OpCodes.Ldstr)
                        {
                            ...
                            instr.Insert(i + 2, Instruction.Create(OpCodes.Call, ObfuscateExe.Init2));
                            instr.Insert(i + 3, Instruction.Create(OpCodes.Call, ObfuscateExe.Init));
                            instr.RemoveAt(i);
                        }
                    }
